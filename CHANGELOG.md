...
- Generated `Where` structs now include `Has<Rel>(filters ...bob.Mod[*dialect.SelectQuery])` methods that filter rows by the existence of related rows, using a correlated `EXISTS` subquery (semi-join) built from the schema's relationship definitions. Unlike filtering through `SelectJoins`, this does not multiply parent rows, so `DISTINCT` is not needed and `Count()`/`LIMIT`-based pagination stay correct ([#722](https://github.com/stephenafamo/bob/pull/722)). (thanks @sandonemaki)
