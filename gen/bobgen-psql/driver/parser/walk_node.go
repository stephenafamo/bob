...
func (w *walker) walk(a any) nodeInfo {
	...
	case *pg.SelectStmt:
		...
		info = w.reflectWalk(reflect.ValueOf(a))
		info.start = w.getStartOfTokenBefore(info.start, pg.Token_SELECT)
	...
	case *pg.Alias:
		...
		info = info.addChild("Aliasname", aliasNameInfo)
...
func (w walker) findIdentOrUnreserved(pos int32) nodeInfo {
	return w.findTokenAfterFunc(pos, func(t *pg.ScanToken) bool {
...
func (w walker) findTokenAfter(position int32, tokens ...pg.Token) nodeInfo {
	return w.findTokenAfterFunc(position, func(t *pg.ScanToken) bool {
		return slices.Contains(tokens, t.Token)
	})
}

func (w walker) findTokenAfterFunc(position int32, f func(*pg.ScanToken) bool) nodeInfo {
